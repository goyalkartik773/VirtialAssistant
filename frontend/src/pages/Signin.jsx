...
function Signup(){
    ...
    return (
        ...
        <form className="absolute left-[100px] w-[90%] h-[600px] max-w-[500px] backdrop-blur-[12px] shadow-lg shadow-[#01012e] flex flex-col items-center justify-center gap-[20px] px-[20px]" onSubmit={handleSignIn}>
        <h1 className="text-white text-[30px] font-semibold pb-[20px]" >SignIn to <span className="text-[#55ff00]"> Virtual Assistant</span>  </h1>
        ...
        {!showPassword && <FiEyeOff className="absolute top-[290px] right-[50px] w-[25px] h-[25px] text-white cursor-pointer" onClick={()=>setShowPassword(true)}/>}
        {showPassword && <FiEye  className="absolute top-[290px] right-[50px] w-[25px] h-[25px] text-white cursor-pointer" onClick={()=>setShowPassword(false)}/>}
        </div>
        <button className="min-w-[150px] h-[60px] text-black font-semibold bg-white rounded-full text-[19px] mt-[30px] cursor-pointer" disabled={loading}>{loading?"loading...":"Sign In"}</button>
